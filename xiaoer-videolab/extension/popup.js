...
async function onQueued(url, title) {
  ...
  // The daemon already opens the monitor page (macOS `open`) when it accepts
  // the job. Wait a beat, then focus that tab instead of duplicating it.
...
// Called directly from the popup so it works even if the service worker is asleep.
