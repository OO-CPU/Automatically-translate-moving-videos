...
# Track active download processes so they can be cancelled / paused.
_active_downloads: dict[str, subprocess.Popen] = {}
_active_downloads_lock = threading.Lock()
# File path (as yt-dlp/curl writes it, e.g. the .part) -> download key (URL).
_active_files: dict[str, str] = {}
# Download key -> {platform, date}: registered at thread start so pause/cancel
# by filename works even before yt-dlp prints its "Destination" line.
_active_meta: dict[str, dict] = {}
# Download key -> total video size in bytes (parsed from yt-dlp progress lines).
_active_total: dict[str, int] = {}
# URLs cancelled via the UI (pause/delete). Their threads must not write a
# "failed" history entry when the process is killed.
_cancelled: set[str] = set()
# AppleScript automation permission state: None=unknown, True/False.
# After repeated failures we stop asking, so a declined permission prompt
# doesn't stall every download.
_osascript_ok = None
_osascript_failures = 0
...
def cancel_download(url: str, silent: bool = False) -> bool:
    """Kill a running download process by URL.

    With silent=True the download thread suppresses its "failed" history entry
    (used for pause / delete from the monitor page). Returns True if a process
    was killed.
    """
...
def _register_file(text: str, url: str) -> None:
    """Register file paths yt-dlp announces it is writing, keyed by URL.

    yt-dlp prints "Destination: xxx.mp4" but actually writes "xxx.mp4.part",
    so both spellings are registered — the monitor page knows the .part path.
    """
    paths = set()
    for m in re.finditer(r"\[download\] Destination:\s*(.+?)\s*$", text, re.MULTILINE):
        paths.add(m.group(1).strip())
    for m in re.finditer(r'\[Merger\]\s+Merging formats into "(.+)"', text):
        paths.add(m.group(1).strip())
    for p in paths:
        if not p:
            continue
        with _active_downloads_lock:
            _active_files[p] = url
            if p.lower().endswith(".part"):
                _active_files[p[:-5]] = url
            else:
                _active_files[p + ".part"] = url


_TOTAL_RE = re.compile(
    r"\[download\]\s+\d+(?:\.\d+)?%\s+of\s+(?:~)?([\d.]+)\s*(KiB|MiB|GiB)"
)


def _parse_total_size(line: str):
    """从 yt-dlp 进度行解析视频总大小（字节）；解析不到返回 None。"""
    m = _TOTAL_RE.search(line)
    if not m:
        return None
    size = float(m.group(1))
    mult = {"KiB": 1024, "MiB": 1024 ** 2, "GiB": 1024 ** 3}[m.group(2)]
    return int(size * mult)


def _find_url_by_path(path: str) -> str:
    """Map a written file path back to its download URL.

    Tries the exact paths registered from yt-dlp's output first, then falls back
    to a name match against every running download's {platform}_{title}_{date}
    pattern — this works even before yt-dlp announces the Destination line.
    """
    base = Path(path).name
    with _active_downloads_lock:
        if path in _active_files:
            return _active_files[path]
        for url, meta in reversed(list(_active_meta.items())):
            pfx = f"{meta.get('platform', '')}_"
            d = meta.get("date", "")
            if base.startswith(pfx) and (not d or f"_{d}" in base):
                return url
    return ""
...
_PROXY_PLATFORMS = {"YouTube", "TikTok", "X", "Vimeo", "Instagram",
                 "Facebook", "Reddit", "Dailymotion"}


def download(url: str, filename: str = "") -> None:
    with _active_downloads_lock:
        if url in _active_downloads:
            log(f"[duplicate] {url} already downloading — ignored")
            return
        _active_downloads[url] = None  # reserve the key before Popen
    log(f"[start] {url}")
    ...
    if filename:
        # Resume with an exact name (from a paused download): keeps the same
        # .part so yt-dlp continues where it stopped, even across midnight.
        output_tpl = str(DOWNLOADS / _safe_filename(filename))
    ...
    try:
        # yt-dlp writes its progress/log lines to stderr, so both pipes are
        # drained by threads: collect the lines for history parsing AND learn
        # the written file paths as they appear (pause/cancel by filename).
        ...
        def _drain(stream) -> None:
            try:
                for line in stream:
                    ...
                    _register_file(line, url)
                    total = _parse_total_size(line)
                    if total:
                        with _active_downloads_lock:
                            _active_total[url] = total
            except Exception:
                pass
        ...
        with _active_downloads_lock:
            ...
            _active_meta[url] = {"platform": platform, "date": date}
        try:
            rc = proc.wait(timeout=3600)
            result = subprocess.CompletedProcess(cmd, rc, "".join(out_lines), "")
        finally:
            with _active_downloads_lock:
                _active_downloads.pop(url, None)
                _active_meta.pop(url, None)
                _active_total.pop(url, None)
                for k in [k for k, v in _active_files.items() if v == url]:
                    del _active_files[k]
        ...
        if result.returncode == 0:
            ...
            with _active_downloads_lock:
                silent = url in _cancelled
                _cancelled.discard(url)
            if silent:
                log(f"[cancelled] {url} (silent, .part kept)")
            else:
...
def download_direct(direct_url: str, referer: str, filename: str, page_url: str) -> None:
    ...
    """
    with _active_downloads_lock:
        if page_url in _active_downloads:
            log(f"[duplicate] {page_url} already downloading — ignored")
            return
        _active_downloads[page_url] = None  # reserve the key before Popen
    ...
    try:
        ...
        with _active_downloads_lock:
            ...
            _active_files[str(out)] = page_url
            _active_meta[page_url] = {"platform": platform, "date": date}
        try:
            _, stderr = proc.communicate(timeout=3600)
        finally:
            with _active_downloads_lock:
                _active_downloads.pop(page_url, None)
                _active_meta.pop(page_url, None)
                for k in [k for k, v in _active_files.items() if v == page_url]:
                    del _active_files[k]
        ...
        if ok:
            ...
            with _active_downloads_lock:
                silent = page_url in _cancelled
                _cancelled.discard(page_url)
            if silent:
                log(f"[cancelled-direct] {page_url} (silent, partial kept)")
            else:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/health":
            ...
            self.end_headers()
            self.wfile.write(body)
        elif path == "/active-totals":
            with _active_downloads_lock:
                totals = {}
                for p, u in _active_files.items():
                    total = _active_total.get(u)
                    if total:
                        totals[p] = total
            body = json.dumps({"totals": totals}, ensure_ascii=False).encode("utf-8")
            self.send_response(200)
            self._cors()
            self.send_header("Content-Type", "application/json")
            self.send_header("Content-Length", str(len(body)))
    ...
    def _open_dashboard(self) -> None:
        """Best-effort: open the download monitor page in Chrome.

        Prefers AppleScript so an already-open tab is focused instead of
        duplicated; falls back to `open` if scripting Chrome is not allowed
        (e.g. the macOS automation permission was declined).
        """
        global _osascript_ok, _osascript_failures
        script = (
            'tell application "Google Chrome"\n'
            '  repeat with w in windows\n'
            '    set i to 1\n'
            '    repeat with t in tabs of w\n'
            '      if URL of t starts with "http://localhost:7799" then\n'
            ...
            '    end repeat\n'
            '  end repeat\n'
            '  open location "http://localhost:7799"\n'
            'end tell'
        )
        if _osascript_ok is not False:
            try:
                p = subprocess.run(
                    ["osascript", "-e", script],
                ...
                if p.returncode == 0:
                    ...
                    _osascript_failures = 0
                    return
                _osascript_failures += 1
                log(f"[dashboard] osascript rc={p.returncode} ({_osascript_failures}/3)")
            except Exception as e:
                _osascript_failures += 1
                log(f"[dashboard] osascript failed ({e}) ({_osascript_failures}/3)")
            if _osascript_failures >= 3:
                _osascript_ok = False
                log("[dashboard] disabling osascript for this session")
        try:
            subprocess.Popen(
                ["open", "-a", "Google Chrome", "http://localhost:7799"],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )
        except Exception as e:
            log(f"[dashboard] failed to open monitor page: {e}")
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        with _active_downloads_lock:
            ...
            if proc and proc.poll() is None:
                ...
                self.send_header("Content-Type", "application/json")
                self.end_headers()
                self.wfile.write(b'{"queued":true,"duplicate":true}')
                return

        threading.Thread(target=download, args=(url, filename), daemon=True).start()
    ...
    def _handle_download_direct(self) -> None:
        ...
        with _active_downloads_lock:
            proc = _active_downloads.get(page_url)
            if proc and proc.poll() is None:
                log(f"[duplicate] {page_url} already downloading — ignored")
                self.send_response(202)
                self._cors()
    ...
    def _handle_cancel(self) -> None:
        """Cancel a running download by URL or by written file path.

        Payload: {url} or {path}. `path` is matched against the files yt-dlp /
        curl are currently writing (see _active_files), so the monitor page can
        act on records it only knows by filename. Always silent: no "failed"
        history entry is produced, and .part files are kept for resume.
        """
        origin = self.headers.get("Origin", "")
        if origin.startswith(("http://", "https://")):
            self.send_response(403)
        ...
        path = data.get("path", "")
        if path and not url:
            url = _find_url_by_path(path)
        if not url:
            self.send_response(404)
            self._cors()
            self.send_header("Content-Type", "application/json")
            self.end_headers()
            self.wfile.write(b'{"cancelled":false,"url":""}')
            return
        killed = cancel_download(url, silent=True)
        body = json.dumps({"cancelled": killed, "url": url}, ensure_ascii=False).encode("utf-8")
        self.send_response(200)
        self._cors()
        self.send_header("Content-Type", "application/json")
        self.send_header("Content-Length", str(len(body)))
